...
public class paccountpaymentframe extends JFrame {
	...
	public paccountpaymentframe() {
		...
		// 계좌 이체 결제 이미지 ImageIcon
		ImageIcon accountpaymentIcon = new ImageIcon(
				"D:\\JAVA Programming\\GYM_pjt\\pap.png");
		Image accountpayment = accountpaymentIcon.getImage();
		Image changeaccountpayment = accountpayment.getScaledInstance(370, 250, Image.SCALE_SMOOTH);
		...
		// 기간 들어가는 JLabel
		JLabel mojb = new JLabel("P.T 회원권 (" + ptchooseframe.month + "개월)");
		mojb.setFont(new Font("Bauhaus", Font.BOLD, 18));
		mojb.setBounds(160, 93, 200, 20);
		mojb.setOpaque(true);
		mojb.setForeground(Color.RED);
		mojb.setBackground(Color.white);
		c.add(mojb);

		// 가격 들어가는 JLabel
		JLabel prjb = new JLabel(ptchooseframe.price + "원");
		prjb.setFont(new Font("Bauhaus", Font.BOLD, 18));
		prjb.setBounds(160, 133, 100, 20);
		prjb.setOpaque(true);
		prjb.setForeground(Color.RED);
		...
		// 계좌 이체 결제 정보 넣는 JLabel
		JLabel cpjb = new JLabel(changeaccountpaymentIcon);
		...
		pjb.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (phonenumber.length() != 13) {
					...
					paccountpaymentsuccessframe Paccountpaymentsuccessframe = new paccountpaymentsuccessframe();
					Paccountpaymentsuccessframe.setVisible(true);
					setVisible(false);
				}
			}
		});
	...
	public void updateDateTimejb() {
		...
		if (ptchooseframe.month != 12) {
			cal.add(Calendar.MONTH, ptchooseframe.month);
		} else {
